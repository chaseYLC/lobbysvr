	...
	namespace Log
	{
		...
		bool Logger::InitBoostLog()
		{
			...
			if (true == _config._useLogFile)
			{
				...
				oss << _config._dirName << "/" << _config._fileName;	// set_filter �� ���� set_file_collector �� log folder ������ �ȵ�. �׷��� ���⼭ directory ��ε� �־���.
				if (!ec) {
					oss << "_" << hostName;
				}
		...
		void Logger::Dump(void* dwOffset, size_t size, const char* dump_title) {
			...
			if (dump_title) {
				if (strlen(dump_title) >= (TMP_SIZE - 100))
					strncpy(dump_bar, " start-----\r\n", TMP_SIZE - 1);
				else {
					strncpy(dump_bar, dump_title, TMP_SIZE - 1);
					strncpy(dump_bar, " start-----\r\n", TMP_SIZE - 1);
				}
